...
- Constants (32KB ring buffer, 64-byte bulk packet size)

### firmware (`picostream-firmware`)
...
Cargo features:
- `rp2040` - Build for RP2040 (Pico, Pico W)
...
- Input sources: stdin (default), file (`-i`/`--file`), or string (`-d`/`--data`)
- Input formats: `bits` (ASCII '0'/'1' characters, whitespace ignored, default) or `bytes` (packed bytes)
- Loop mode (`-l`/`--loop`): continuously repeat file or string input
...
```bash
# Stream from stdin (default)
cat data.bin | ./target/release/picostream -s 100000

# Stream from file
./target/release/picostream -s 1000000 -i data.bin

# Stream from string (bytes)
./target/release/picostream -s 1000000 -d $'\x55\xAA'

# Stream from string (bits format)
./target/release/picostream -s 1000000 -f bits -d "01010101 10101010"

# Loop a file continuously (Ctrl-C to stop)
./target/release/picostream -s 1000000 -i pattern.bin --loop

# Loop a bit pattern continuously
./target/release/picostream -s 1000000 -f bits -d "01010101" --loop

# Continuous 500kHz square wave via stdin
cat /dev/zero | tr '\0' $'\x55' | ./target/release/picostream -s 1000000
```
...
- Create udev rule: `/etc/udev/rules.d/99-picostream.rules`
```
...
- Saves timestamped logs to `/tmp/picostream-{firmware,client}-YYYYMMDD_HHMMSS.log`
